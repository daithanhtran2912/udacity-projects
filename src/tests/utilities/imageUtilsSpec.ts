...
describe("Testing image valiation utility", (): void => {
  ...
  it("should throw error when filename does not exist", async (): Promise<void> => {
    await expectAsync(processImage("test", height, width)).toBeRejectedWithError(
      "File name: test does not exist!"
    );
  });

  it("should throw error when height is not a number", async (): Promise<void> => {
    const strHeight = "height" as unknown as number;
    await expectAsync(processImage(filename, strHeight, width)).toBeRejectedWithError(
      "Either height is empty or not a number!"
    );
  });
  ...
  it("should throw error when height is smaller than 1", async (): Promise<void> => {
    await expectAsync(processImage(filename, 0, width)).toBeRejectedWithError(
  ...
  it("should throw error when width is not a number", async (): Promise<void> => {
    ...
    await expectAsync(processImage(filename, height, strWidth)).toBeRejectedWithError(
      "Either width is empty or not a number!"
    );
  });
  ...
  it("should throw error when width is smaller than 1", async (): Promise<void> => {
    await expectAsync(processImage(filename, height, 0)).toBeRejectedWithError(
